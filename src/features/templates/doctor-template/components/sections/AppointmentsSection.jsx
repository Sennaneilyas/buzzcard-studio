...
export function AppointmentsSection() {
  ...
  return (
    ...
    >
      ...
      <div className="mt-8 bg-white p-6 rounded-[24px] border-[0.667px] border-[#4682b42e] shadow-[0px_4px_12px_rgba(70,130,180,0.08)] relative overflow-hidden min-h-[340px]">
        
        {step > 1 && step < 4 && (
          <button onClick={prevStep} className="absolute top-5 left-5 text-[#4682b4] hover:opacity-70 z-10">
        ...
        {step < 4 && (
          <div className="flex justify-center gap-1.5 mb-6 mt-1">
            {[1, 2, 3].map(i => (
              <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${step >= i ? "w-6 bg-[#4682b4]" : "w-1.5 bg-[#4682b433]"}`} />
            ))}
          </div>
        )}
                ...
                <div className="grid grid-cols-2 gap-2 max-h-[220px] overflow-y-auto pr-1 pb-2 custom-scrollbar">
                  {availableDays.map((date, i) => (
                    <button
                      ...
                      className="border-[0.667px] border-[#4682b42e] text-[#4682b4] rounded-[12px] p-3 text-[13px] font-medium hover:bg-[#4682b4] hover:text-white transition-colors"
                    >
                ...
                <div className="grid grid-cols-2 gap-2">
                  {timeSlots.map(time => (
                    <button
                      ...
                      className="border-[0.667px] border-[#4682b42e] text-[#4682b4] rounded-[12px] p-2.5 text-[13px] font-medium hover:bg-[#4682b4] hover:text-white transition-colors"
                    >
                      {time}
                    </button>
                  ))}
