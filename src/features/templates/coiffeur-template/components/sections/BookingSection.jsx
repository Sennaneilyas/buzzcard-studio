      ...
      days.push(
        <button
          ...
          className={`h-9 w-9 rounded-full flex items-center justify-center text-[13px] font-medium transition-colors mx-auto ${
            isDisabled ? "text-gray-300 cursor-not-allowed" : "text-[#1A1A1A] hover:bg-[#C5A880]/15"
          } ${isSelected ? "bg-[#C5A880] text-white hover:bg-[#C5A880]" : ""}`}
        >
          {i}
        </button>
      );
  ...
  return (
    ...
    >
      ...
      <div className="mt-8 bg-white/90 backdrop-blur-md w-full max-w-[400px] p-6 rounded-[24px] border border-black/5 shadow-xl relative overflow-hidden min-h-[360px]">
        ...
        {step > 1 && step < 4 && (
          <button onClick={prevStep} className="absolute top-5 left-5 text-[#C5A880] hover:text-[#1A1A1A] transition-colors z-10">
        ...
        {step < 4 && (
          <div className="flex justify-center gap-1.5 mb-6 mt-1">
            {[1, 2, 3].map(i => (
              <div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${step >= i ? "w-6 bg-[#C5A880]" : "w-1.5 bg-[#C5A880]/20"}`} />
            ))}
          </div>
        )}
          ...
          <AnimatePresence custom={direction} mode="wait">
              ...
              <motion.div key="step1" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3 }} className="flex flex-col">
                ...
                <div className="w-full select-none">
                  <div className="flex justify-between items-center mb-4 px-2">
                    <button onClick={prevMonth} className="text-[#C5A880] hover:text-[#1A1A1A] transition-colors p-1"><ChevronLeft className="w-4 h-4"/></button>
                    <span className="font-medium text-[14px] text-[#1A1A1A]">{monthNames[month]} {year}</span>
                    <button onClick={nextMonth} className="text-[#C5A880] hover:text-[#1A1A1A] transition-colors p-1"><ChevronRight className="w-4 h-4"/></button>
                  </div>
            ...
            {step === 2 && (
              <motion.div key="step2" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3 }} className="flex flex-col">
                ...
                <div className="flex items-center justify-center gap-2 mb-6 text-[12px] text-[#C5A880] bg-[#C5A880]/10 py-1.5 px-3 rounded-full w-max mx-auto">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {timeSlots.map(time => (
                    <button
                      ...
                      className="border border-black/5 text-[#1A1A1A] rounded-[12px] p-2.5 text-[13px] font-medium hover:bg-[#C5A880] hover:text-white hover:border-[#C5A880] transition-colors"
                    >
                      {time}
                    </button>
                  ))}
            ...
            {step === 3 && (
              <motion.div key="step3" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3 }} className="flex flex-col">
                ...
                <div className="flex flex-col gap-2 mb-5 text-[12px] text-[#C5A880] bg-[#C5A880]/10 p-3 rounded-[12px]">
                ...
                <form onSubmit={handleBook} className="flex flex-col gap-3">
                        ...
                        >
                          {profile?.services?.map(s => (
                            <div 
                              ...
                              className="flex items-center gap-3 p-3 hover:bg-[#C5A880]/10 cursor-pointer transition-colors border-b border-black/5 last:border-0"
                            >
                              ...
                              <div className="flex flex-col">
                                ...
                                <span className="text-[#C5A880] text-[12px] font-semibold">{s.price}</span>
                              </div>
                            </div>
                          ))}
                        </motion.div>
                  ...
                  <div className="relative">
                    ...
                    <input required type="text" placeholder="Nom complet" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} className="w-full pl-10 pr-3.5 py-3 sm:py-2.5 bg-white border border-black/5 rounded-[13px] text-[16px] sm:text-[13px] focus:outline-none focus:border-[#C5A880] transition-all" />
                  </div>
                  <div className="relative">
                    <Phone className="absolute left-3.5 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                    <input required type="tel" placeholder="Numéro de téléphone" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} className="w-full pl-10 pr-3.5 py-3 sm:py-2.5 bg-white border border-black/5 rounded-[13px] text-[16px] sm:text-[13px] focus:outline-none focus:border-[#C5A880] transition-all" />
                  </div>
                  <button type="submit" className="w-full bg-[#1A1A1A] text-white font-semibold py-3 sm:py-2.5 rounded-[13px] text-[14px] sm:text-[13px] hover:bg-[#C5A880] transition-colors mt-2 active:scale-[0.98]">
                    Confirmer le RDV
                  </button>
                </form>
              </motion.div>
            ...
            {step === 4 && (
              <motion.div key="step4" custom={direction} variants={slideVariants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3 }} className="flex flex-col items-center text-center mt-4">
                ...
                <button onClick={() => { setStep(1); setDirection(-1); setFormData({name:"", phone:"", service:""}); }} className="text-[13px] font-semibold text-[#C5A880] hover:text-[#1A1A1A] transition-colors">
                  Nouveau rendez-vous
                </button>
              </motion.div>
            )}
