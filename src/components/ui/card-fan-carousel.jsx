...
function getSlotConfig(totalCards, slot) {
  ...
  return {
    ...
    zIndex: 10 - Math.floor(Math.abs(slot - center)),
  };
}
...
export default function SocialCards({ cards }) {
  ...
  useEffect(() => {
      ...
      visibleEntries.forEach(({ el, slot }) => {
        ...
        if (hoveredSlot !== null) {
          ...
          if (slot === hoveredSlot) {
            ...
            const pushStrength = 5 * (1 - Math.abs(normalized)) * (1 + 0.2 * Math.max(0, 3 - distance));
            ...
            if (slot === visibleEntries.length - 1 && hoveredSlot < centerSlot) targetY -= 0.5 * hM;
            if (slot === 0 && hoveredSlot > centerSlot) targetY -= 0.5 * hM;
          }
        } else {
        ...
        gsap.to(el, {
          ...
          duration: 0.4, delay, ease: "power3.out", overwrite: "auto",
        });
    ...
    const enterHandlers = visibleEntries.map(({ el, slot }) => {
      ...
      return { el, handler, onMouseLeave };
    });
    ...
    return () => {
      enterHandlers.forEach(({ el, handler, onMouseLeave }) => {
  ...
  return (
    <div className="flex flex-col items-center w-full py-0 lg:py-2 px-4 md:px-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] h-[280px] md:h-[350px]">
          {cards.map((card, index) => {
            const image = (
              <div className="relative w-[150px] sm:w-[180px] md:w-[220px] aspect-[4/5] rounded-[1.5rem] bg-white p-2 sm:p-2.5 shadow-[0_15px_40px_rgba(0,0,0,0.12)] border border-black/5 transition-transform duration-500 will-change-transform">
                <div className="relative w-full h-full rounded-xl overflow-hidden">
                  <img src={card.imgUrl} loading="lazy" alt={card.alt || `Card ${index}`} className="w-full h-full object-cover z-10 transition-transform duration-700 group-hover:scale-110" />
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/10 to-transparent opacity-90 pointer-events-none" />
                  <div className="absolute bottom-4 left-4 right-4 text-white font-bold text-sm sm:text-base leading-tight pointer-events-none transition-transform duration-300 group-hover:-translate-y-1">{card.alt}</div>
                  
                  {/* Hover Link Indicator */}
                  <div className="absolute top-3 right-3 bg-white/20 backdrop-blur-md rounded-full p-2 opacity-0 group-hover:opacity-100 transition-all duration-300 transform group-hover:scale-100 scale-75 border border-white/20">
